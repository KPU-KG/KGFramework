...
void KG::GameFramework::OnTestInit()
{
	...
	static KG::Core::GameObject testSpotLightObjects2;
	...
	for ( size_t y = 0; y < cdas; y++ )
	{
		for ( size_t x = 0; x < cdas; x++ )
		{
			size_t index = cdas * y + x;
			{
				...
				testCubeObjects[index].GetComponent<KG::Component::TransformComponent>()->Translate(x * 2.5, 0.0f, y * 2.5);
...

	{
		...
		light->SetPointLight(DirectX::XMFLOAT3(1.75f, 0.0f, 0.0f), 0.1f, 7.5f);
		...
		testPointLightObjects[index].GetComponent<KG::Component::TransformComponent>()->Translate(1.0f, 0.0f, 1.0f);
	...
	}
	{
		...
		light->SetPointLight(DirectX::XMFLOAT3(0.0f, 1.75f, 0.0f), 0.1f, 7.5f);

		testPointLightObjects[index].AddComponent(tran);
		testPointLightObjects[index].AddComponent(light);
		testPointLightObjects[index].AddComponent(sdw);
		testPointLightObjects[index].AddComponent(lam);
		testPointLightObjects[index].GetComponent<KG::Component::TransformComponent>()->Translate(11.0f, 0.0f, 11.0f);
	}
	{
		int index = 2;
		auto* tran = this->system->transformSystem.GetNewComponent();
		...
		light->SetPointLight(DirectX::XMFLOAT3(0.0f, 0.0f, 1.75f), 0.01f, 7.5f);

		testPointLightObjects[index].AddComponent(tran);
		testPointLightObjects[index].AddComponent(light);
		testPointLightObjects[index].AddComponent(sdw);
		testPointLightObjects[index].AddComponent(lam);
		testPointLightObjects[index].GetComponent<KG::Component::TransformComponent>()->Translate(19.0f, 0.0f, 19.0f);
	}

	{
		...
		light->SetSpotLight(DirectX::XMFLOAT3(2.0f, 2.0f, 0.0f), 10.0f,
		...
		testSpotLightObjects.GetComponent<KG::Component::TransformComponent>()->Translate(-3.0f, 0.0f, 25.0f);
		tran->RotateEuler(0, 90 + 45, 0);
...

	{
		...
		testSpotLightObjects2.name = "Light2";


		light->SetSpotLight(DirectX::XMFLOAT3(2.0f, 0.0f, 2.0f), 10.0f,
			DirectX::XMConvertToRadians(120.0f), DirectX::XMConvertToRadians(30.0f), 4.0f);

		testSpotLightObjects2.AddComponent(tran);
		testSpotLightObjects2.AddComponent(light);
		testSpotLightObjects2.AddComponent(sdw);
		testSpotLightObjects2.AddComponent(lam);
		testSpotLightObjects2.GetComponent<KG::Component::TransformComponent>()->Translate(+25.0f, 0.0f, -3.0f);
		tran->RotateEuler(0, -45, 0);
		//tran->Translate( 0.5f, 2.0f, 0.5f );
		//mainCamera->GetGameObject()->GetTransform()->AddChild( testSpotLightObjects.GetTransform() );
	}
