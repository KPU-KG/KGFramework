...
bool KG::Component::MaterialComponent::OnDrawGUI()
{
    ...
    ImGui::Separator();
    if (ImGui::TreeNode("Material Edit"))
    {
        for (size_t i = 0; i < this->shaders.size(); i++)
        {
            for (size_t j = 0; j < this->shaders[i]->MaterialDescription.size(); j++)
            {
                ImGui::PushID(count++);
                auto element = this->shaders[i]->GetMaterialElement(this->materialIndexs[i]);
                this->shaders[i]->MaterialDescription[j].DrawGUI(element);
                ImGui::PopID();
            }
        }
        ImGui::TreePop();
