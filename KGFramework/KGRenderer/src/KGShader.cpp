...
KG::Resource::DynamicElementInterface KG::Renderer::Shader::GetMaterialElement(UINT index)
{
    return this->materialBuffer->GetElement(index);
}
D3D12_RASTERIZER_DESC KG::Renderer::Shader::CreateRasterizerState(ShaderMeshType meshType, ShaderPixelType pixType, ShaderGeometryType geoType)
{
    ...
    if (pixType == ShaderPixelType::GSCubeShadow || pixType == ShaderPixelType::Shadow || pixType == ShaderPixelType::Transparent || pixType == ShaderPixelType::Add)
    {
        d3dRasterizerDesc.CullMode = D3D12_CULL_MODE_NONE;
    }
...
void KG::Renderer::MaterialElement::DrawGUI(Resource::DynamicElementInterface& data)
{
    static KG::Utill::HashString id;
    static std::string log;
    if (ImGui::TreeNode(this->comment.c_str()))
    {
        switch (type)
        {
        case KG::Renderer::Texture:
        {
            //ImGui::Text("Not Supported");
            auto handle = KG::Renderer::KGDXRenderer::GetInstance()->GetDescriptorHeapManager()->GetGPUHandle(data.Get<UINT>(this->offset));
            auto width = ImGui::GetColumnWidth() - 10;
            ImGui::TextureView((ImTextureID)handle.ptr, ImVec2(width, width), this->comment.c_str());
            ImGui::InputHashString("Change ID", &id);
            ImGui::SameLine();
            if (ImGui::SmallButton("apply"))
            {
                auto* result = KG::Resource::ResourceContainer::GetInstance()->LoadTexture(id);
                if (result != nullptr)
                {
                    data.Set<UINT>(this->offset, result->index);
                    log = "Texture Set Complete";
                }
                else
                {
        ...
        break;
        case KG::Renderer::FLOAT1:
        {
            float result = data.Get<float>(this->offset);
            if (ImGui::DragFloat("Value", &result, 0.01f))
            {
                data.Set<float>(this->offset, result);
            }
        }
        break;
        case KG::Renderer::FLOAT2:
        {
            XMFLOAT2 result = data.Get<XMFLOAT2>(this->offset);
            if (ImGui::DragFloat2("Value", &(float&)result, 0.01f))
            {
                data.Set<XMFLOAT2>(this->offset, result);
            }
        }
        break;
        case KG::Renderer::FLOAT3:
        {
            XMFLOAT3 result = data.Get<XMFLOAT3>(this->offset);
            if (ImGui::DragFloat3("Value", &(float&)result, 0.01f))
            {
                data.Set<XMFLOAT3>(this->offset, result);
            }
        }
        break;
        case KG::Renderer::FLOAT4:
        {
            XMFLOAT4 result = data.Get<XMFLOAT4>(this->offset);
            if (ImGui::DragFloat4("Value", &(float&)result, 0.01f))
            {
